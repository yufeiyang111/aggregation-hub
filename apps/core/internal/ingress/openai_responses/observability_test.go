...
func TestHandlerRecordsResponsesLifecycle(t *testing.T) {
	...
	handler.ServeHTTP(response, httptest.NewRequest(http.MethodPost, "/v1/responses", strings.NewReader(`{"model":"demo/model-a","input":"hello"}`)))
	if response.Code != http.StatusOK || recorder.start.SourceProtocol != observability.ProtocolOpenAIResponses || recorder.start.Endpoint != "/v1/responses" || recorder.lifecycle.completed != 1 {
...
func (observationGateway) Complete(context.Context, normalize.NormalizedRequest) (normalize.NormalizedResponse, error) {
	return normalize.NormalizedResponse{ID: "resp_obs", Model: "demo/model-a", Parts: []normalize.ContentPart{normalize.TextPart{Text: "ok"}}, FinishReason: normalize.FinishReasonStop}, nil
}
...
func TestHandlerRecordsResponsesStreamingLifecycle(t *testing.T) {
	recorder := &observationRecorder{lifecycle: &observationLifecycle{}}
	...
	response := httptest.NewRecorder()
	handler.ServeHTTP(response, httptest.NewRequest(http.MethodPost, "/v1/responses", strings.NewReader(`{"model":"demo/model-a","stream":true,"input":"hello"}`)))
...
func (observationStreamingGateway) Stream(ctx context.Context, _ normalize.NormalizedRequest, emit normalize.StreamEmitter) error {
	for _, event := range []normalize.NormalizedEvent{normalize.ResponseStartEvent{ResponseID: "resp_stream", Model: "demo/model-a"}, normalize.ResponseEndEvent{FinishReason: normalize.FinishReasonStop}} {
