...
func TestHandlerRecordsOpenAIChatLifecycle(t *testing.T) {
	...
	handler.ServeHTTP(response, httptest.NewRequest(http.MethodPost, "/v1/chat/completions", strings.NewReader(`{"model":"demo/model-a","messages":[{"role":"user","content":"hello"}]}`)))
	if response.Code != http.StatusOK || recorder.start.SourceProtocol != observability.ProtocolOpenAIChat || recorder.start.Endpoint != "/v1/chat/completions" || recorder.lifecycle.completed != 1 {
...
func (observationGateway) Complete(context.Context, normalize.NormalizedRequest) (normalize.NormalizedResponse, error) {
	return normalize.NormalizedResponse{ID: "chat_obs", Model: "demo/model-a", Parts: []normalize.ContentPart{normalize.TextPart{Text: "ok"}}, FinishReason: normalize.FinishReasonStop}, nil
}
...
func TestHandlerRecordsOpenAIChatStreamingLifecycle(t *testing.T) {
	recorder := &observationRecorder{lifecycle: &observationLifecycle{}}
	...
	response := httptest.NewRecorder()
	handler.ServeHTTP(response, httptest.NewRequest(http.MethodPost, "/v1/chat/completions", strings.NewReader(`{"model":"demo/model-a","stream":true,"messages":[{"role":"user","content":"hello"}]}`)))
...
func (observationStreamingGateway) Stream(ctx context.Context, _ normalize.NormalizedRequest, emit normalize.StreamEmitter) error {
	for _, event := range []normalize.NormalizedEvent{normalize.ResponseStartEvent{ResponseID: "chat_stream", Model: "demo/model-a"}, normalize.ResponseEndEvent{FinishReason: normalize.FinishReasonStop}} {
