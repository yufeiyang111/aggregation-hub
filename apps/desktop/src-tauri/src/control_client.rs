...
pub(crate) fn post_json<Request: Serialize, Response: DeserializeOwned>(
    ...
    let body = serde_json::to_vec(payload).map_err(|_| "管理请求编码失败".to_owned())?;
    execute_json(control_url, token, "POST", path, Some(&body))
}
...
mod tests {
    ...
    fn accepts_only_internal_paths() {
        ...
        assert!(!valid_path("/internal/v1/providers?cursor=unsafe"));
    }
