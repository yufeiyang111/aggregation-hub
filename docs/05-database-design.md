...
历史预发布兼容仅限一个已知的 `0001_initial.sql` 校验和：启动时还必须确认数据库完整性正常、仅记录初始迁移、`provider_models`/`usage_daily` 仍处于旧基线且尚未出现后续迁移字段。满足这些条件才允许继续执行 `0002` 及之后的前向迁移；不改写历史校验和，不接受任意校验和漂移，其他不匹配仍会停止启动并保留原库。
