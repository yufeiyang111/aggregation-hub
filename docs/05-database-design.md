...
历史预发布兼容仅限一个已知的 `0001_initial.sql` 校验和：启动时必须确认数据库完整性正常、旧初始表结构仍可识别，并核对 `schema_migrations` 中除 `0001` 外的每条记录都与当前嵌入迁移完全一致。尚未执行后续迁移的旧库还必须不存在后续迁移字段；已经完成后续迁移的旧库则允许继续启动。满足这些条件才允许继续执行缺失的前向迁移；不改写历史校验和，不接受任意校验和漂移，其他不匹配仍会停止启动并保留原库。
